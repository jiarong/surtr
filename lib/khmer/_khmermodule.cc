...
static PyObject * hashbits_get_percentile_count(PyObject * self, PyObject * args)
{
    khmer_KHashbitsObject * me = (khmer_KHashbitsObject *) self;
    Hashbits * hashbits = me->hashbits;
    ...
    if (!PyArg_ParseTuple(args, "sf", &long_str, &perc)) {
        return NULL;
    }
    ...
    if (perc > 100 || perc < 0) {
        PyErr_SetString(PyExc_ValueError,
                        "percentile must be in range of [0,100]");
        return NULL;
    }

    BoundedCounterType perc_count = 0;

    hashbits->get_percentile_count(long_str, perc_count, perc);

    return Py_BuildValue("i", perc_count);
}
...
static PyMethodDef khmer_hashbits_methods[] = {
    ...
    { "get_percentile_count", hashbits_get_percentile_count, METH_VARARGS, "Get the percentile of the k-mer counts in the string" },
    {NULL, NULL, 0, NULL}           /* sentinel */
};
